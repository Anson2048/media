...
(function() {
  ...
  let api = [
    ...
    { name: 'czjx8', url: 'http://czjx8.com/?url=' },
    { name: 'wandhi', url: 'http://vip.wandhi.com/?v=' }
  ];
    ...
    showButton: function() {
      if (
        ...
        api.forEach((val, index) => {
          $('.h-ol').append(
            `<li><a target="_blank" href="${val.url +
              encodeURI(location.href)}">${val.name}</a></li>`
          );
        });
        //反馈按钮
        $('.h-ol').append(
          `<li><a target="_blank" style="color: #999;" href="https://github.com/syhyz1990/media/issues">点击反馈</a></li>`
        );
        $('.h-ol').append(github);
